...
Object.defineProperty(exports, "useScrollToItemDynamic", {
    enumerable: true,
    get: function() {
        return useScrollToItemDynamic;
    }
});
...
function useScrollToItemDynamic({ axis, reversed, gap, scrollViewRef, getItemSize, getTotalSize, getOffsetForIndex, measureRefObject, maxCorrections = DEFAULT_MAX_CORRECTIONS, stabilityTimeout = DEFAULT_STABILITY_TIMEOUT, setFlaggedIndex, onOperationComplete, onOperationCancel }) {
    const operationRef = _react.useRef(null);
    const scrollListenerRef = _react.useRef(null);
    const operationIdRef = _react.useRef(0);
    const scheduleCorrectionRef = _react.useRef(null);
    const scheduleStabilityCheckRef = _react.useRef(null);
    const scheduleCorrection = _react.useCallback((operation)=>{
        scheduleCorrectionRef.current == null ? void 0 : scheduleCorrectionRef.current.call(scheduleCorrectionRef, operation);
    }, []);
    const scheduleStabilityCheck = _react.useCallback((operation)=>{
        scheduleStabilityCheckRef.current == null ? void 0 : scheduleStabilityCheckRef.current.call(scheduleStabilityCheckRef, operation);
    }, []);
    const [setStabilityTimeout, clearStabilityTimeoutFn] = (0, _reactutilities.useTimeout)();
    const [requestProgrammaticFrame] = (0, _reactutilities.useAnimationFrame)();
    const [requestCorrectionFrame, cancelCorrectionFrame] = (0, _reactutilities.useAnimationFrame)();
    const clearStabilityTimeout = _react.useCallback((operation)=>{
    ...
    const cancelScheduledFrame = _react.useCallback((operation)=>{
    ...
    const detachScrollListener = _react.useCallback(()=>{
   ...
   */ const clearOperation = _react.useCallback((reason)=>{
   ...
   */ const getTargetElement = _react.useCallback((index)=>{
        const element = measureRefObject.current[index.toString()];
        return element != null ? element : null;
    }, [
        measureRefObject
    ]);
    const ensureScrollListener = _react.useCallback(()=>{
   ...
   */ const evaluateTargetAlignment = _react.useCallback((index)=>{
        const scrollView = scrollViewRef.current;
    ...
    const scheduleProgrammaticScrollReset = _react.useCallback((operation)=>{
   ...
   */ const applyScrollByDelta = _react.useCallback((operation, delta)=>{
        const scrollView = scrollViewRef.current;
   ...
   */ const performScroll = _react.useCallback((operation, behavior)=>{
        const scrollView = scrollViewRef.current;
        if (!scrollView) {
            return;
        }
   ...
   */ const finalizeOperation = _react.useCallback((operation)=>{
   ...
   */ const handleItemMeasured = _react.useCallback((index, _size, delta)=>{
        const active = operationRef.current;
        if (!active) {
            return;
        }
   ...
   */ const handleRendered = _react.useCallback((index)=>{
        const active = operationRef.current;
   ...
   */ const start = _react.useCallback((index, behavior, callback)=>{
   ...
   */ const isActive = _react.useCallback(()=>{
