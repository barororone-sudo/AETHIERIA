...
import { VirtualizerExample, VirtualizerComboboxExample } from './VirtualizerExample.component-browser-spec';
test.use({
    viewport: {
        width: 800,
        height: 600
...
test.describe('Virtualizer', ()=>{
    test('should render only visible items initially', async ({ mount })=>{
        const component = await mount(/*#__PURE__*/ React.createElement(VirtualizerExample, {
            numItems: 1000
        }));
    ...
    test('should have correct ARIA attributes', async ({ mount })=>{
        ...
        await expect(container).toHaveAttribute('aria-label', 'Virtualizer Example');
    ...
    test('should handle rapid scrolling without whitespace', async ({ mount })=>{
        const component = await mount(/*#__PURE__*/ React.createElement(VirtualizerExample, {
            numItems: 1000
        }));
        const container = component.getByTestId('scroll-container');
    ...
    test('should handle small lists (no virtualization needed)', async ({ mount })=>{
        const component = await mount(/*#__PURE__*/ React.createElement(VirtualizerExample, {
            numItems: 5
        }));
    ...
    test('should start with empty list and prepend items without React key errors', async ({ mount, page })=>{
    ...
    test('should handle prepending multiple items without React key errors', async ({ mount, page })=>{
        const consoleErrors = [];
        page.on('console', (msg)=>{
            if (msg.type() === 'error') {
                consoleErrors.push(msg.text());
    ...
    test('should correctly update when scrolled and items are prepended', async ({ mount })=>{
        const component = await mount(/*#__PURE__*/ React.createElement(VirtualizerExample, {
            numItems: 50
        }));
        await expect(component.getByTestId('scroll-container')).toBeVisible();
...
test.describe('ComboboxVirtualizer', ()=>{
    test('should render combobox with virtualized options', async ({ mount, page })=>{
        const component = await mount(/*#__PURE__*/ React.createElement(VirtualizerComboboxExample, null));
